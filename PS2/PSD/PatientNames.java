...
class PatientNames {
	...
	HashMap<String, Integer> patientToGender = new HashMap<String, Integer>(); 	//to store the gender of patients
	...
	void AddPatient(String patientName, int gender) {
		if (gender == 1) {
			...
			patientToGender.put(patientName, gender);
		} else {
	...
	void RemovePatient(String patientName) {
		if (patientToGender.get(patientName) == 1) {
...
class AVL{
	...
	//############# for deletion ###########
	public AVLNode search(AVLNode node, String patientName) {
		if (node == null) {
			return null; //not found
		} else if (node._key.compareTo(patientName) == 0) {
			return node; //found the node
		} else if (node._key.compareTo(patientName) < 0) { // item is located to the right
			return search(node._right, patientName); //search right
		} else { //item is located to the left
			return search(node._left,patientName); //search left
		}
	}

	public String search(String patientName) {			//overload
		AVLNode result = search(_root,patientName);
		if (result == null) { 							//not found
			return null;
		} else {
			return result._key; 						//found, return String
		}
	}	

	public String findMin(AVLNode node) {
		if (node == null) {
			throw new NoSuchElementException("Tree empty");
		} else if (node._left==null) {
			return node._key; 							//this is the min
		} else {
			return findMin(node._left); 				//keep going left till the min
		}
	}

	public String findMin() { 							//overload
		return findMin(_root);
	}

	public String successor(AVLNode node) {
		if (node._right != null) { 						//If can go right
			return findMin(node._right); 				//successor is the min of right subtree
		} else {
			AVLNode par = node._parent;
			AVLNode curr = node;
			//if parent is not root and current is still the right children
			while ((par!= null) && curr._key.compareTo(par._right._key) == 0) {
				curr = par; 							//keep moving up
				par = curr._parent; 					//update the parent
			}
			if (par != null) {
				//curr no longer right children of parent, means successor found
				return par._key;
			} else {
				return "no successor";
			}
	...
	public String successor(String patientName) { //overload
		AVLNode patientPos = search(_root, patientName);
		if (patientPos != null) {
			return successor(patientPos);
		} else {
			return "no successor";
		}
	}

	public AVLNode delete(AVLNode node, String patientName) {
		if (node == null) { 											//item not found
			return node;
		}

		if (node._key.compareTo(patientName) == 0) { 					//found the item to delete
			if (node._left == null && node._right == null) { 			//node is a leaf
				node = null; 											//remove the node
				return null;
			} else if (node._left == null && node._right != null) { 	//1 child at right
				node._right._parent = node._parent;
				node = node._right; 									//bypass node
			} else if (node._right == null && node._left != null) { 	//1 child at left
				node._left._parent = node._parent;
				node = node._left; 										//bypass node
			} else { 													//2 child
				String successorNode = successor(node); 				//find the successor
				node._key = successorNode;								//replace with successor
				node._right = delete(node._right,successorNode);		//delete the successor to prevent duplicates
			}
		} else if (node._key.compareTo(patientName) > 0) { 				//go to the left
			node._left = delete(node._left,patientName);
		} else { 														//go to the right
			node._right = delete(node._right,patientName);
		}

		if (node != null) { 											//not a leaf node that is deleted
			node = balanceTree(node);									// Balancing the tree
	...
	public void delete(String patientName) { //overload
		_root = delete(_root,patientName);
	}
	//####################### for deletion #######################
}
